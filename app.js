...
function initCompanyDetails() {
  ...
  function showDetails(company) {
    ...
    if (mockData.transactions) {
      const companyTransactions = mockData.transactions.filter(t => t.company === company.name || t.companyId === company.id);
      companyTransactions.forEach(t => {
        if (t.user && !assignedUsers.find(u => u.username === t.user) && !transactionUsers.find(u => u.username === t.user)) {
          // Create a mock user object from transaction data
          transactionUsers.push({
            username: t.user,
            firstName: t.user.split(' ')[0] || t.user,
            surname: t.user.split(' ')[1] || '',
...
function renderAuthorizations() {
  ...
  mbTbody.innerHTML = pendingBilling.length === 0
    ...
    <tr>
      ...
      <td>
        <button class="btn btn-sm btn-primary" onclick="approveManualBilling('${item.id}')">Approve</button>
        <button class="btn btn-sm btn-secondary" onclick="rejectManualBilling('${item.id}')">Reject</button>
      </td>
    </tr>
    `).join('');
  ...
  batchesTbody.innerHTML = pendingBatches.length === 0
    ...
    <tr>
      ...
      <td>
        <button class="btn btn-sm btn-primary" onclick="approveBatch('${batch.id}')">Approve</button>
        <button class="btn btn-sm btn-secondary" onclick="rejectBatch('${batch.id}')">Reject</button>
      </td>
    </tr>
    `).join('');
